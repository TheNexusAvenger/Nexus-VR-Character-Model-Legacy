...
File: NexusVRCharacterModel/NexusVRCharacter/Character/FootPlanter.module.lua
Author: TheNexusAvenger
Date: March 11th 2018



FootPlanter:CreateSolver(CenterPart)
	Creates foot planter solver given a HumanoidRootPart
	RETURNS: FootPlanterSolver
	
CLASS FootPlanterSolver
	FootPlanterClass:GetFeetCFrames()
		Gets CFrames of feet
		RETURNS: CFrame,CFrame
	FootPlanterClass:OffsetFeet(Offset)
		Moves feet by an offset Vector3. Used when teleporting
...
function FootPlanter:CreateSolver(CenterPart)
	...
	local LEG_GAP = 1.2
	--
	local STRIDE_FORWARD = 1.7
	local STRIDE_BACKWARD = 3.3
	local STRIDE_HEIGHT = 0.6
	local STRIDE_RESTING = 1
	--
	local WALK_SPEED_VR_THRESHOLD = 4
	...
	--[[function FootPlanterClass:SetScale(Scale)
		local NewScale = Scale/CurrentScale
		
		LEG_GAP = LEG_GAP * NewScale
		STRIDE_FORWARD = STRIDE_FORWARD * NewScale
		STRIDE_BACKWARD = STRIDE_BACKWARD * NewScale
		STRIDE_HEIGHT = STRIDE_HEIGHT * NewScale
		STRIDE_RESTING = STRIDE_RESTING * NewScale
	end]]
